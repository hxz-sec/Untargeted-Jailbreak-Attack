...
def get_decode_args():
    ...
    parser.add_argument("--guard-device", type=int, default=1)
    parser.add_argument("--response-device", type=int, default=0)
    ...
    parser.add_argument("--logits-lr", type=float, default=0.1)
    ...
    for step in range(1, max_steps + 1):
        ...
        if step % 10 == 0 or step == max_steps:
            print(
                ...
                f"Unsafe: {unsafe_score:.3f} "
                # f"| Sem: {semantic_loss.item():.2f} | "
...
def build_grad_transfer_matrix(guard_tok,
                               llama_tok,
                               llama_model,
                               dtype=torch.float32):
    """
    返回形状 (G_vocab, L_vocab_model) 的稀疏矩阵 T：
        guard_grad_llama = guard_grad @ T
    其中 L_vocab_model = llama_model.get_input_embeddings().weight.size(0)
    """
    G = guard_tok.vocab_size
    L = llama_model.get_input_embeddings().weight.size(0)   
    rows, cols, vals = [], [], []

    for gid in range(G):
        tok = guard_tok.convert_ids_to_tokens(gid)
        tok = tok[1:] if tok.startswith("Ġ") else tok      
...
def decode_suffix(
    ...
    for iter in range(args.num_iters):
        ...
        L_common = min(init_logits.size(1), guard_grad_llama.size(0))  # 50 vs 78 → 50
        ...
        print("stage2 time: ", time.time() - stage2_start)
        ...
        if iter % 5 == 0 or iter+1 == args.num_iters:
            ...
            # response = response_cold_vllm(prompt,generation_model)
            print("response time: ", time.time() - response_start)
...
def main():
    ...
    print("generation_model_name:",generation_model_name)
    ...
    elif "Qwen2.5-7B-Instruct" in args.generation_model: model_path = "Qwen-2.5-7B-Instruct"
    ...
    if "advbench" in args.dataset:
        ...
        dataset_name = "harmBench"
    elif "strongreject" in args.dataset:
        dataset_name = "strongreject"        
    else:
    ...
    iters = args.retry_amount * args.num_iters
    output_file = os.path.join(
        output_dir, 
        guard_path,
        model_path,
        f"{dataset_name}_untarget_{guard_path}_{iters}_early_100.jsonl"
    ...
    for i, question in tqdm(enumerate(harmful_questions), total=len(harmful_questions), desc="Decoding"):
        ...
        # })
    end_time = time.time()
    print(f"总计耗时：{end_time - start_time:.3f} 秒")
